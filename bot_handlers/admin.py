...
async def send_welcome_to_channel(message: Message):
    """
    Отправляет приветственное сообщение с главным меню в чат.
    ...
    # Получаем стандартную клавиатуру без админ-кнопки
    keyboard = kb.get_main_inline_keyboard(is_admin=False)
    
    # Отправляем сообщение в чат, откуда пришла команда
    await message.answer(welcome_text, parse_mode="Markdown", reply_markup=keyboard)

    # Если это супергруппа или канал, можно попробовать удалить исходное сообщение с командой
    # чтобы не засорять чат.
    try:
        await message.delete()
    except Exception:
        # Не удалось удалить сообщение (например, нет прав или это личный чат)
        # В этом случае просто игнорируем ошибку
        pass 